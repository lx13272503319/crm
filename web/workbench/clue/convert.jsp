...
<script type="text/javascript">
	$(function(){
				...
				$.ajax({
					...
					//	[{市场1},{2},{3}]
						...
						$.each(data,function (i,obj){
							html+= '<tr>'
							html+= '<td><input type="radio" value="'+obj.id+'" name="xz"/></td>'
							html+= '<td id="N'+obj.id+'">'+obj.name+'</td>'
		...
		$("#submitActivityBtn").click(function (){
			//获取选中的id和name
			var aId=$("input[name=xz]:checked").val()
			var aName=$("#N"+aId).html()

			//为隐藏域赋上选中的id，为文本框赋上name
			$("#activityId").val(aId)
		...
		$("#convertBtn").click(function (){
			...
			//	为了安全和方便，使用post传统请求提交（name为key,value为value）
				$("#tranForm").submit()
			}else {
				//不需要创建交易，只需要clueId
